...
const getAll = async (filters = {}, selectedFields = {}) => {
	console.log(filters);
	const donations = await Recurring.find(filters, selectedFields)
		.populate({
			path: 'campaign',
			model: 'Campaigns',
...
const DBGetRecurringTaskList = async (filters) => {
	try {
		const recurring = await Recurring.find({
			$or: [
				{ recurringCount: 0 },
				{ $expr: { $lt: ['$currentRecurringCount', '$recurringCount'] } },
			],
			$and: [{ isActive: true }],
		}).populate({
