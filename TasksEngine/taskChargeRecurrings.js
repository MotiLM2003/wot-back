...
const taskChargeRecurrings = async (task) => {
	...
	for (const donation of list) {
		...
		const nextPaymentDate = moment(donation.lastPaymentDate).add(
			options.days,
			'days'
		);
		...
		if (isPayment) {
			...
			if (
				...
				donation.currentRecurringCount >= donation.recurringCount
			) {
			...
			updateById({
				...
				currentRecurringCount: donation.currentRecurringCount,
